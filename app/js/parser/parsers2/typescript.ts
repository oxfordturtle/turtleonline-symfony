/**
 * Parser 2 for Turtle TypeScript.
 */
import { simpleStatement, expression, typeCheck, variableAssignment } from './common'
...
export default function TypeScript (program: Program): void {
...
function eosCheck (routine: Program|Subroutine): void {
  if (routine.lexemes[routine.lex]) {
    if (routine.lexemes[routine.lex].content !== ';' && routine.lexemes[routine.lex].type !== 'newline') {
      throw new CompilerError('Statement must be followed by a semicolon or placed on a new line.', routine.lexemes[routine.lex - 1])
    }
...
function forStatement (routine: Program|Subroutine): ForStatement {
  ...
  if (!routine.lexemes[routine.lex] || routine.lexemes[routine.lex].content !== ';') {
    throw new CompilerError('"for (..." must be followed by a semicolon.', routine.lexemes[routine.lex - 1])
  }
  routine.lex += 1
  ...
  if (!(change instanceof VariableAssignment)) {
    throw new CompilerError('"for (...;" must be followed by a loop variable reassignment.', routine.lexemes[routine.lex - 1])
  }
  ...
  // expecting a closing bracket
  if (!routine.lexemes[routine.lex] || routine.lexemes[routine.lex].content !== ')') {
    throw new CompilerError('"for (...; ...; ..." must be followed by a closing bracket ")".', routine.lexemes[routine.lex - 1])
  }
  routine.lex += 1
