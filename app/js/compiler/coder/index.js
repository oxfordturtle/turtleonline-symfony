...
const compileSubroutines = (routines, startLine, language) => {
  ...
  while (index < routines.length) {
    ...
    const innerCode = compileInnerCode(routines[index], startLine + offset, language)
    const subroutineCode = pcoder.subroutine(routines[index], innerCode)
...
const compileInnerCode = (routine, startLine, language) => {
  ...
  while (lex < routine.lexemes.length) {
    const result = coders[language].call(null, routine, lex, startLine + pcode.length)
