 ...
 * the main PROGRAM, and any subsequent elements are its SUBROUTINES, in the
 * order in which they need to be compiled
 *
 * look at the tools/factory module to see what the PROGRAM and SUBROUTINE
 * objects look like
 *
...
export default function (lexemes: Lexeme[], language: Language) {
  const parsers = { BASIC, Pascal, Python }
  if (lexemes.length === 0) {
    throw new CompilerError('Program does not contain any lexemes.')
  }
...
// set the address of the variables for a routine, and determine how much memory
// it needs
function fixVariableMemory (routine): void {
  let memoryNeeded = 0
...
// subroutine pointers needed (number of subroutines, plus 1 for the return
// value if there's at least one function)
function subroutinePointers (subroutines) {
  return subroutines.some(x => x.type === 'function')
